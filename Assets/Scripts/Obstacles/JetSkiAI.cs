...
public class JetSkiAI : MonoBehaviour
{
    ...
    [SerializeField] private float detectionDistanceObstacle = 5f;  // Дистанция для Obstacle
    ...
    private void AvoidObstacles()
    {
        ...
        if (Physics.Raycast(rayOriginLower, transform.forward, out hitLower, detectionDistanceObstacle) && hitLower.collider.CompareTag("Obstacle"))
        {
            TurnAway();
        }
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawRay(transform.position, transform.forward * detectionDistanceObstacle);
